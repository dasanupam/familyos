...
export default function Travel() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      await api.post("/travel/trips", { ...form, member_id: form.member_id || defaultMemberId, budget: form.budget ? Number(form.budget) : null });
      setShowAdd(false); setForm({});
      refresh(); toast.success("Trip added");
  ...
  return (
    <div className="space-y-6" data-testid="travel-page">
      <div className="flex items-end justify-between flex-wrap gap-4">
        ...
        <button onClick={() => setShowAdd(true)} data-testid="add-trip-button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {trips.map((t) => (
          <TripCard key={t.id} trip={t} onDelete={() => remove(t.id)} memberName={members.find((m) => m.id === t.member_id)?.name} />
        ))}
      </div>
      ...
      {showAdd && (
        <Modal title="New trip" onClose={() => setShowAdd(false)}>
          <form onSubmit={submit} className="space-y-3" data-testid="trip-form">
            ...
            <button className="w-full bg-[#184A31] text-white py-2.5 rounded-full font-medium" data-testid="trip-save-button">Save trip</button>
          </form>
        </Modal>
      )}
    </div>
...
function TripCard({ trip, onDelete, memberName }) {
