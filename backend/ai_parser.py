...
Output schema (strict JSON, no prose, no markdown fences):
{
  ...
  "transactions": [
     {"member": "<name|null>", "date": "YYYY-MM-DD", "amount": <number, positive>, "type": "expense"|"income",
  ...
  "loans": [
     {"member": "<name|null>", "name": "<string>", "outstanding": <number>, "emi": <number|null>, "rate": <number|null>}
  ],
  "lab_results": [
     {"member": "<name|null>", "date": "YYYY-MM-DD", "test": "<test name>",
      "value": <number|null>, "unit": "<string>", "reference_range": "<string|null>"}
  ],
  "prescriptions": [
  ...
  "insurance_policies": [
     {"member": "<name|null>", "name": "<policy name>", "policy_type": "life"|"health"|"term"|"vehicle"|"home"|"travel"|"other",
      "provider": "<string|null>", "sum_assured": <number|null>, "premium": <number|null>,
  ...
  "assets": [
     {"member": "<name|null>", "name": "<string>", "kind": "property"|"vehicle"|"gold"|"other",
      "current_value": <number|null>, "purchase_value": <number|null>, "purchase_date": "YYYY-MM-DD|null", "notes": "<string|null>"}
  ],
  "trips": [
  ...
  "goals": [
     {"member": "<name|null>", "name": "<string>", "target_amount": <number>, "current_amount": <number|null>,
  ...
  "plans": [
     {"member": "<name|null>", "name": "<unique plan name>", "plan_type": "financial"|"retirement"|"investment"|"budget"|"supplement"|"diet"|"fitness"|"treatment"|"other",
      ...
      "items": [{"title": "<string>", "detail": "<string|null>", "amount": <number|null>, "due_date": "YYYY-MM-DD|null"}]}
  ],
  "generic_entries": [
...
def _context_block(today_iso: str, member_names: list[str], corrections: list[str] | None) -> str:
    lines = [
...
def _generate(contents, today_iso: str, member_names: list[str], corrections: list[str] | None) -> dict:
...
async def parse_universal(content: str, today_iso: str, member_names: list[str],
                          corrections: list[str] | None = None) -> dict:
    user_text = f"INPUT:\n{content[:60000]}\n\nReturn strict JSON now."
    return _generate(user_text, today_iso, member_names, corrections)


async def parse_pdf_file(data: bytes, today_iso: str, member_names: list[str],
                         corrections: list[str] | None = None) -> dict:
...
async def parse_image_file(data: bytes, mime_type: str, today_iso: str, member_names: list[str],
                           corrections: list[str] | None = None) -> dict:
    from google.genai import types
    contents = [
